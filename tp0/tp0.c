...
int connect_to_server(char * ip, char * port) {
  ...
  int server_socket = socket(hints.ai_family, hints.ai_socktype, hints.ai_protocol );

  // 3. Conectemosnos al server a traves del socket! Para eso vamos a usar connect()
  int retorno = connect( server_socket, (struct sockaddr *)&hints , sizeof(hints));
...
void  wait_hello(int socket) {
  ...
  */
   int result_recv = recv(socket,buffer,sizeof(buffer), MSG_WAITALL);
  /*
